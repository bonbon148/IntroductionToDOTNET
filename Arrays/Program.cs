...
namespace Arrays
{
    internal class Program
    {
        ...
        {
            //Garbage collector - он удаляет объест из динамической памяти, после удаления ПОСЛЕДНЕЙ ссылки на этот объект.
#if ARRAYS_1
            int[] arr = { 3, 5, 8, 13, 21 };
            ...
            Console.WriteLine(); 
            //Сумма, средне-арифметическое, минимальное и максимальное значение.
            Console.WriteLine($"Сумма: {arr.Sum()}");
            Console.WriteLine($"Средне-арифмитическое: {arr.Average()}");
            ...
            Console.WriteLine($"Средне-арифмитическое: {i_arr_2.Cast<int>().ToArray().Average()}");
...
#endif
#if JAGGED_ARRAYS
            //Зубчатый массив организован в память точно так же,
            //как и двумернный динамический массив в языке С++
